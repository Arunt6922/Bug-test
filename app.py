...
def start_ping(data):
    domain = data['domain']
    with subprocess.Popen(['ping', '-n', '4', domain], stdout=subprocess.PIPE, bufsize=1, universal_newlines=True) as p:
...
def start_openports(data):
    domain = data['domain']
    open_ports = subprocess.run(['C:\\Program Files (x86)\\Nmap\\nmap.exe', '-F', domain], stdout=subprocess.PIPE, text=True)
